...
const checkX = (puzzle) => {
    ...
    if (Object.keys(valuesLtoR).length===puzzle.length && Object.keys(valuesRtoL).length===puzzle.length) return true;
    return false;
}
...
const checkY = (puzzle) => {
    ...
    if (Object.keys(rightY).length===puzzle.length && Object.keys(leftY).length===puzzle.length) return true;
    return false;
}
...
const validate = (puzzle, row, col) => {
	...
	for (let i = 0; i < size * size; ++i)
	  ...
	  for (let i = size * Math.floor(row / size); i < size * Math.floor(row / size) + size; ++i)
	    for (let j = size * Math.floor(col / size); j < size * Math.floor(col / size) + size; ++j)
	      if (puzzle[i][j] != 0)
...
const solve = (puzzle) => {
	...
	while(index < stackList.length) {
		const impossible = validate(puzzle, stackList[index].row, stackList[index].col);
		
		// Populates stack for possible values
		for (let i = puzzleSize - 1; i >= 0; --i)
	      {
	        if (impossible[i] == 0)
	          stackList[index].stack.push(i+1);
	      }
		...
		puzzle[stackList[index].row][stackList[index].col] = stackList[index].stack[stackList[index].stack.length-1]

		if (index === stackList.length - 1) {
			// TO DO: CHECK X AND Y
			solutions.push({puzzle: puzzle.map(row => row.map(col => col)), x: checkX(puzzle), y: checkY(puzzle) });
